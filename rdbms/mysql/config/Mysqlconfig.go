...
func (c *Mysqlconfig) Values(driverName, hostName string, port int64, database, userName, password string, maxOpenconns, maxIdleConns, maxLifeTimeConn int) {
...
func (c *Mysqlconfig) Connect() (*sql.DB, error) {
	...
	if err == nil {
		...
		log.Printf("Successfully connected to: %s", c.Database)
	}
	return db, err
}
